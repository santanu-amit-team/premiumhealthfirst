...
<head>
    ...
    <style>
        .progress .part div:last-child, .progress .bar { background: #a42a5b; }
                    ...
                    .logo { background-image: url('<?= $path['images'] ?>/logo-up.png?5221549'); }
        ...
        <div class="container">
              <form name="is-upsell" class="is-upsell" accept-charset="utf-8" enctype="application/x-www-form-urlencoded;charset=utf-8" style="display: none;">
                <input type="hidden" name="limelight_charset" id="limelight_charset" value="utf-8" />
                 <?php if($customer['cardType']=='master'){?>
            <input type="hidden" name="campaigns[2][id]" value="10">
            <?php }else{ ?>
            <input type="hidden" name="campaigns[2][id]" value="4">
            <?php } ?>
            ...
            <div id="upsell-box">
                ...
                <div class="center top">
                    ...
                    <p>Customers that purchased <strong><?= $get_data['productName']['step1'] ?></strong> also purchased <strong><?= $get_data['productName']['step2'] ?></strong>
                    </p>
                </div>
                <div class="coupon-container clearfix" onclick="$('[name=is-upsell]').submit();">
                    ...
                    <div class="bottle-info">
                        ...
                        <p class="line-3">with
                            <br><strong><?= $get_data['productName']['step2'] ?></strong>
                        </p>
                        <p class="line-4">Add a <strong>SPECIAL TRIAL</strong> bottle
                            <br>just pay S&amp;H of</p>
                        <p class="line-5"><span class='upsell-msrp'></span> $
                            <?= $campaignDetails[4]['shipping_price']; ?>
                        </p>
                    </div>
                </div>
                <div class="center">
                ...
                <p>*Due to limited inventory levels on any given day, we must limit trial sales to 250 maximum per day. Representations regarding the efficacy and safety of
                    <?= $get_data['productName']['step1'] ?> and have not been evaluated by the Food and Drug Administration. The FDA only evaluates foods and drugs, not supplements like these products. These products are not intended to diagnose, prevent, treat, or cure any disease.
                        <br>
                        <br>This product has not been evaluated by the FDA. This product is not intended to diagnose, treat, cure, or prevent any disease. This product is intended to be used in conjunction with a healthy diet and regular exercise. Consult your physician before starting any diet, exercise program, and taking any diet pill to avoid any health issues.</p>
                <p class="decline"> <a href="<?= get_no_thank_you_link() ?>">No Thanks, I decline this offer</a>
                </p>
            </div>
            <div style='position: absolute; overflow: hidden; width: 1px; height: 1px;'></div>
            <footer>
                <div class="footer">
                    <p class="copyright">&copy;
                        <?= date('Y');?>
                            <?= $get_data['siteDetails']['siteTitle'] ?>&mdash; All rights reserved.</p>
                    <p class="customerservice">Customer Service:
                        <a href="tel:866-529-5649"><?= $get_data['siteDetails']['phone'] ?></a>
                    </p>
                    <p class="footerlinks"> 
