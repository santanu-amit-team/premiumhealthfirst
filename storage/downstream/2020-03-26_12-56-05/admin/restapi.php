...
{
    if ($table_name == 'extension-managers' || $table_name == 'extensions') {
        return;
    }
    ...
    $prev_content = md5(json_encode(
        Database::table($table_name)
            ->where('id', '=', $id)->findAll()->asArray()[0]
    ));
    $curr_content = md5(json_encode(Request::form()->all()));
...
if ($action_information['method_name'] == 'edit') {
    version_control(
        $action_information['table_name'], $action_information['arguments'][0]
    );
}

$logMethods = array('edit', 'add', 'delete', 'editManageData');
